...
const CreateRegionPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validation logic now primarily used to set message
        if (isFormInvalid) {
            setMessage('Error: Please fill out all mandatory fields (*), including complete phone numbers.');
            return;
        }
    ...
    // A constant to check form validity on every render.
    // The form is invalid if ANY of these conditions are true.
    const isFormInvalid = 
        !selectedCountry ||
        !address.trim() ||
        phone.length < 7 ||
            ...
            <div className="w-full max-w-4xl bg-white/70 backdrop-blur-xl border border-white/30 rounded-2xl shadow-2xl shadow-blue-900/10">
                <form onSubmit={handleSubmit} className="p-8 sm:p-12 space-y-8">
                    ...
                    <div className="space-y-6">
                        <fieldset className="border border-slate-300 rounded-lg p-6">
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-600 mb-2">
                                    Select Country/Region <span className="text-red-500">*</span>
                                </label>
                                <Select
                        ...
                        <fieldset className="border border-slate-300 rounded-lg p-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-600 mb-2">
                                        WhatsApp (Sales Chat) <span className="text-red-500">*</span>
                                    </label>
                                    <PhoneInput
                                        country={selectedCountry ? selectedCountry.value.toLowerCase() : 'us'}
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-600 mb-2">
                                        WhatsApp (Support Chat) <span className="text-red-500">*</span>
                                    </label>
                                    <PhoneInput
                                        country={selectedCountry ? selectedCountry.value.toLowerCase() : 'us'}
                                ...
                                <div className='md:col-span-2 border-t pt-6 mt-2'>
                                    <h3 className='text-md font-semibold text-gray-500 mb-4 text-center'>Optional Links</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between pt-6 gap-4 border-t border-slate-200">
                        ...
                        <button 
                            type="submit" 
                            disabled={isFormInvalid || isSubmitting} // Button is disabled if form is invalid OR if it's currently submitting
